...
Dim score : score = RequestSet("score" , "POST" , "" )
...
Sub Update()
	...
	SQL = "SET NOCOUNT ON; " &_
	"declare @Score INT,@State int,@Idx int,@Snumber VARCHAR(50),@appCount varchar(3),@SnumberCheck INT,@ProgramIdx int , @AreaIdx int ;" &_
  "set @Score   = ? " &_
	"set @State   = ? " &_
	"set @Idx     = ? " &_
	"set @Snumber = ? " &_
	...
	with objCmd
		.CommandText = SQL
    .Parameters.Append .CreateParameter( "@Score"      ,adInteger , adParamInput , 0  , score )
...
'Sub PushScore()
'합격 점수 넣기
'  SQL = "DECLARE @Score INT" &_
'        "SET @Score = 0"     &_
'        "UPDATE [dbo].[SP_PROGRAM_APP] SET [Score] = @Score WHERE [Idx] = @Idx"
'        
'  call cmdopen()
'  with objCmd
'    .CommandText = SQL
