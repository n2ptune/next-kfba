...
Dim sClass     : sClass     = RequestSet("sClass"   , "GET" , "")
 '합격 점수
Dim sScore     : sScore     = RequestSet("sScore"   , "GET" , "")
...
Call dbopen()
  ...
  "<option value=""9"" "&IIF(sState="9","selected","")&" >면제</option>" &_
...
Sub GetList()
    ...
    .Parameters("@sScore").value   = sScore
...
If cntList > -1 Then 
  ...
  ElseIf arrList(FI_State,iLoop) = "9" Then
    StateTxt = "<font color=""#9a1134"">면제</font>"
	ElseIf arrList(FI_State,iLoop) = "10" Then 
		StateTxt = "<font color=""#11179a"">합격</font>"
	End If

	PrograName = arrList(FI_ProgramNema,iLoop)
  Score = arrList(FI_Score,iLoop)

  If isNULL(arrList(FI_Score,iLoop)) Or arrList(FI_Score, iLoop) = 0 Then
    Score = "-"
  Else
    Score = Score & "점"
  End If
  ...
  "<option value=""9"" "&IIF(arrList(FI_State,iLoop)="9","selected","")&" >면제</option>" &_
	"<option value=""10"" "&IIF(arrList(FI_State,iLoop)="10","selected","")&" >합격</option>"
		...
		ntpl.setBlockReplace array( _
			...
			,array("UserPhone", trim(arrList(FI_UserHphone1,iLoop)) &"-"& arrList(FI_UserHphone2,iLoop) &"-"& arrList(FI_UserHphone3,iLoop) ) _
