...
Sub Update()
	...
	SQL = "SET NOCOUNT ON; " &_
	"declare @State int,@Idx int,@Snumber VARCHAR(50),@appCount varchar(3),@ProgramIdx int , @AreaIdx int, @Bigo VARCHAR(MAX) , @FirstName varchar(50),@LastName varchar(50),@UserIdx INT ;" &_
	"set @State      = ? " &_
	"set @Idx        = ? " &_
	"set @Snumber    = ? " &_
	"set @Bigo       = ? " &_
