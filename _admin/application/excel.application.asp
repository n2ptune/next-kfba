...
Sub GetList()
	...
	"	INNER JOIN [dbo].[SP_PROGRAM_AREA] D ON(A.[AreaIdx] = D.[Idx] ) "

	SQLWhere = ""
	If(sPcode <> "") Then SQLWhere = SQLWhere  & " AND C.[ProgramIdx] = @sPcode "
	If(sId <> "") Then SQLWhere = SQLWhere  & " AND D.[Name] LIKE '%'+@sArea+'%' " 
	If(sId <> "") Then SQLWhere = SQLWhere  & " AND B.[UserId] LIKE '%'+@sId+'%' "
	If(sName <> "") Then SQLWhere = SQLWhere  & " AND B.[UserName] LIKE '%'+@sName+'%' "
	If(sPhone3 <> "") Then SQLWhere = SQLWhere  & " AND B.[UserHphone3] LIKE '%'+@sPhone3+'%' "
	If(sState <> "") Then SQLWhere = SQLWhere  & " AND A.[State] = @sState "
	If(sSnumber <> "") Then SQLWhere = SQLWhere  & " AND A.[Snumber] = @sSnumber " 
	If(sKind <> "") Then SQLWhere = SQLWhere  & " AND C.[Kind] = @sKind "
	If(sClass <> "") Then SQLWhere = SQLWhere  & " AND C.[Class] = @sClass "
	If(sIndate <> "") Then SQLWhere = SQLWhere  & " AND CONVERT(VARCHAR,A.[InData],23) >= @sIndate "
	If(sOutdate <> "") Then SQLWhere = SQLWhere  & " AND CONVERT(VARCHAR,A.[InData],23) <= @sOutdate "
	If(sOnDate <> "") Then SQLWhere = SQLWhere  & " AND CONVERT(VARCHAR,C.[OnData],23) = @sOnDate "
	If(sOnTime <> "") Then SQLWhere = SQLWhere  & " AND  CONVERT(VARCHAR(2),C.[OnData],108) = @sOnTime "


	If(SQLWhere <> "") Then SQL = SQL & " WHERE " & Right(SQLWhere,Len(SQLWhere)-4)
