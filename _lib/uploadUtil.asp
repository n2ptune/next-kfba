...
Function DextFileUpload(ByVal ControlName,ByVal sFolderName,ByVal s_w)
	...
	if f <> "" then
		...
		' 이미지 파일 일때 썸네일 저장
		If (s_width > 0) And ( LCase(file_ext) = "jpg" Or LCase(file_ext) = "jpeg" Or LCase(file_ext) = "gif" Or LCase(file_ext) = "bmp" Or LCase(file_ext) = "png" ) Then 
...
' ========================================================================
' Function명 : DextFileUploadNoNameChg
' 목      적 : 첨부파일 업로드 및 데이타 처리
' 입      력 : 
' ========================================================================
Function DextFileUploadNoNameChg(ByVal ControlName,ByVal sFolderName,ByVal s_w)
	...
	if f <> "" then
		...
		'arrFileName=Right(strFilePath_new, len(strFilePath_new) - instrRev(strFilePath_new, "/"))
    arrFileName = f.FileName

	Else
		arrFileName=""
	End If
	DextFileUploadNoNameChg=arrFileName
