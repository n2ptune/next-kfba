...
Sub InsertPay()
	...
	"IF @CntDuplicate = 0 AND @CK_StartDate <= GETDATE() AND @CK_EndDate >= GETDATE() AND @CK_MaxNumber > @CK_CNT_APP " &_
	"BEGIN "&_
	...
	with objCmd
		...
		.Parameters.Append .CreateParameter( "@UserIdx"    ,adVarChar , adParamInput,50 , payMethod )
		set objRs = .Execute
	End with
	call cmdclose()
	'프로그램정보
